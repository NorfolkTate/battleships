...
class Board:
    ...
    def __init__(self, size, num_ships, board_type):
        # code explanation courtesy of code institute project scope
        # (ref in README)
        self.size = size
        self.board = [["~" for _ in range(size)] for _ in range(size)]
        # code explanation courtsey of stack overflow (ref in README)
    ...
    def print_board(self, hide_ships=False):
        ...
        # code courtesy of code academy (ref in README)

        for row_number, row in enumerate(self.board, start=1):
            # code courtsey of stack overflow (ref in README)
...
class Play:
    ...
    def validate_guess(self):
        ...
        while True:
            try:
                guess = input("Guess a row between 1 - 5 followed "
                              "by enter, or type 'quit' to exit:\n")
                if guess.lower() == 'quit':
                    self.quit_game()

                guess_row = int(guess) - 1
                guess = input("Guess a column between 1 - 5 followed "
                              "by enter, or type 'quit' to exit:\n")
                if guess.lower() == 'quit':
                    self.quit_game()

                guess_column = int(guess) - 1
                if (0 <= guess_row < self.board.size and
                   0 <= guess_column < self.board.size):
                    return guess_row, guess_column

                else:
                    print("Invalid guess: please enter a "
                          "number between 1 - 5\n")
    ...
    def play_battleships(self, computer_board):
        ...
        # code explanation courtsey of w3 schools (ref in README)
        ...
        while (self.board.hits < self.board.num_ships and
            ...
            if computer_board.hits == computer_board.num_ships:
                print(f"Congratulations {player_name}, you've "
                      "sunk all the computer's battleships! \n")
                print(win_message)
                return

            # computer's turn
            print("Computer's turn:\n")
            computer_row, computer_column = (
                self.computer_turn(computer_has_guessed)
                )
            print(f"Computer guessed row {computer_row + 1} "
                  f"and column {computer_column + 1}\n")
            ...
            # print scoreboard and game board
